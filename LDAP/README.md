...
- Type in the PATH environment variables - `path_to_your_server\php\`. Preliminarily make sure, that on this path exist the files `libeay32.dll` and `ssleay32.dll`.
- OR (most extreme case) Copy the libraries `libeay32.dll` and `ssleay32.dll` into `C:\Windows\System32`
- You need be sure, that on the path of variable 'extension_dir' in the `php.ini` (default value is `extension_dir = путь_к_вашему_серверу\php\ext\`) exist  the library `php_ldap.dll`
